...
function NotificationsScreen(props) {
  ...
  useEffect(() => {
    if (notifications.length === 0) {
      props.getNotifications(props.auth.user.id, 0);
    }
  ...
  const loadMoreItems = (lengthOfItems) => {
    setLoadingMore(true);
    props.getNotifications(props.auth.user.id, lengthOfItems);
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {notifications.length === 0 ? (
        <NoNotifications navigation={props.navigation} />
      ) : (
        <FlatList
          data={notifications}
          onRefresh={() => {
...
export default connect(mapStateToProps, { newNotification, getNotifications })(
  NotificationsScreen
);
